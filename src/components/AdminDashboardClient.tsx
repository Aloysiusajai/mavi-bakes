...
const statusMeta: Record<OrderStatus, { icon: React.ElementType; className: string; label: string }> = {
  Pending: {
    icon: Clock3,
    label: "Pending",
...
export default function AdminDashboardClient() {
  ...
  const fetchOrders = useCallback(
    async ({ silent = false, notify = false }: { silent?: boolean; notify?: boolean } = {}) => {
      ...
      try {
        ...
        const nextOrders = Array.isArray(data) ? data.map(normalizeOrder).filter((order) => order.id) : [];

        if (!hasLoadedRef.current) {
          seenOrderIdsRef.current = new Set(nextOrders.map((order) => order.id));
          hasLoadedRef.current = true;
        } else if (notify) {
          const freshOrders = nextOrders.filter((order) => !seenOrderIdsRef.current.has(order.id));
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      if (dateFilter && dateKey(order.deliveryDate || order.createdAt) !== dateFilter) return false;
  ...
  const runWithLoading = async (id: string, action: () => Promise<void>) => {
  ...
  const updateStatus = async (id: string, status: OrderStatus) => {
    await runWithLoading(id, async () => {
      try {
        ...
        pushToast({ type: "success", title: "Status updated", message: `Order #${shortId(id)} is now ${status}.` });
      } catch (err) {
        console.error(err);
        pushToast({ type: "error", title: "Status update failed", message: "Please try again in a moment." });
      }
    });
  };
      ...
      onConfirm: async () => {
        ...
        await runWithLoading(id, async () => {
          try {
            ...
            pushToast({ type: "success", title: "Order deleted", message: `Order #${shortId(id)} was removed.` });
          } catch (err) {
            console.error(err);
            pushToast({ type: "error", title: "Delete failed", message: "Unable to delete this order." });
          }
        });
      },
    });
  ...
  const sendThankYou = (order: Order) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        if (!phone) {
          pushToast({ type: "error", title: "No phone number", message: "This order does not have a valid phone number." });
          return;
        }
        ...
        await runWithLoading(order.id, async () => {
          try {
            ...
            pushToast({ type: "success", title: "Thank-you marked", message: `Message opened for ${order.customerName || "customer"}.` });
          } catch (err) {
            console.error(err);
            pushToast({ type: "error", title: "Update failed", message: "WhatsApp opened, but the order was not updated." });
          }
        });
      },
    });
  ...
  const sendCakeReady = (order: Order) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        await runWithLoading(order.id, async () => {
          try {
            ...
            pushToast({ type: "success", title: "Customer notified", message: `Order #${shortId(order.id)} is ready.` });
          } catch (err) {
            console.error(err);
            pushToast({ type: "error", title: "Update failed", message: "WhatsApp opened, but the order was not updated." });
          }
  ...
  const exportCSV = () => {
    const header = ["id", "createdAt", "customerName", "email", "phoneNumber", "cakeName", "quantity", "totalPrice", "deliveryAddress"].join(",");
    const rows = orders
      .map((order) =>
        [
          order.id,
    ...
    pushToast({ type: "success", title: "CSV exported", message: `${orders.length} orders downloaded.` });
  };

  const handleLogout = async () => {
    await fetch("/api/admin/logout", { method: "POST" });
    pushToast({ type: "info", title: "Signed out", message: "Redirecting to admin login." });
    router.push("/admin/login");
  };

  const statCards = [
    { label: "Total Orders", value: stats.total, icon: ShoppingBag, tone: "from-[#fff7ed] to-[#fde68a]" },
    { label: "Pending Orders", value: stats.pending, icon: Clock3, tone: "from-[#fff7ed] to-[#fed7aa]" },
    { label: "Ready Orders", value: stats.ready, icon: PackageCheck, tone: "from-[#ecfdf5] to-[#bbf7d0]" },
    { label: "Delivered", value: stats.delivered, icon: Truck, tone: "from-[#f8fafc] to-[#e2e8f0]" },
    { label: "Total Revenue", value: currency.format(stats.revenue), icon: IndianRupee, tone: "from-[#fdf2f8] to-[#fbcfe8]" },
  ];
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#76524c] sm:text-base">
                Track incoming cakes, prep status, customer messages, and delivery flow from one bakery command center.
              </p>
            </div>
        ...
        <section className="rounded-[24px] border border-white/70 bg-white/55 p-4 shadow-[0_20px_60px_rgba(90,47,43,0.09)] backdrop-blur-2xl">
          <div className="grid gap-3 lg:grid-cols-[1fr_auto_auto] lg:items-center">
            <label className="relative block">
              <Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#9a6b62]" size={18} />
              <input
                value={search}
            ...
            <label className="relative block">
              <CalendarDays className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#9a6b62]" size={18} />
              <input
                value={dateFilter}
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold transition ${
                    active ? "bg-[#5a2f2b] text-white shadow-lg shadow-[#5a2f2b]/20" : "bg-white/65 text-[#6e4942] hover:bg-white"
                  }`}
                >
                  {status}
                </button>
...
function StatCard({
  ...
  return (
    <article className={`group rounded-[22px] border border-white/80 bg-gradient-to-br ${tone} p-4 shadow-[0_18px_45px_rgba(90,47,43,0.08)] transition hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(90,47,43,0.14)]`}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#8b5a4e]">{label}</p>
          <p className="mt-2 text-2xl font-black tabular-nums text-[#3f231f]">{value}</p>
        </div>
...
function OrderCard({
  ...
  return (
    ...
    >
      {isNew ? <div className="absolute right-0 top-0 h-24 w-24 rounded-bl-full bg-red-500/10" /> : null}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-black ring-1 ${statusMeta[status].className}`}>
              <StatusIcon size={13} />
              {statusMeta[status].label}
            </span>
            <span className="rounded-full bg-[#5a2f2b]/10 px-2.5 py-1 text-xs font-bold text-[#5a2f2b]">#{shortId(order.id)}</span>
          </div>
          <h2 className="truncate text-xl font-black text-[#3f231f]">{order.customerName || "Unnamed customer"}</h2>
          <p className="mt-1 text-sm font-semibold text-[#7b5a51]">{order.cakeName || "Custom cake"} · Qty {order.quantity ?? 1}</p>
        </div>

        <div className="text-left sm:text-right">
          <p className="text-2xl font-black text-[#3f231f]">{currency.format(Number(order.totalPrice || 0))}</p>
          <p className="mt-1 text-xs font-bold uppercase tracking-[0.14em] text-[#916a60]">Delivery {formatDate(order.deliveryDate)}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-3 text-sm text-[#5f433d] sm:grid-cols-2">
        ...
        <InfoPill label="Address" value={order.deliveryAddress || "No address"} wide />
        {order.customMessage ? <InfoPill label="Message" value={order.customMessage} wide /> : null}
      </div>
      ...
      <div className="mt-5 flex flex-col gap-3 border-t border-[#5a2f2b]/10 pt-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="grid grid-cols-3 gap-2 sm:flex">
          ...
          />
          <ActionButton label="Delete" icon={Trash2} danger disabled={isLoading} onClick={() => onDelete(order.id)} />
        </div>
      </div>
    </article>
  );
...
function InfoPill({ label, value, wide = false }: { label: string; value: string; wide?: boolean }) {
  return (
    <div className={`rounded-2xl border border-white/70 bg-white/55 px-3 py-2 ${wide ? "sm:col-span-2" : ""}`}>
      <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#9a6b62]">{label}</p>
  ...
  return (
    <button
      ...
      className={`inline-flex h-11 items-center justify-center gap-2 rounded-2xl px-3 text-xs font-black transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 ${
        danger ? "bg-red-50 text-red-700 hover:bg-red-100" : "bg-[#5a2f2b] text-white shadow-lg shadow-[#5a2f2b]/15 hover:bg-[#6f3933]"
      }`}
    >
      <Icon size={15} />
  ...
  return (
    <section className="grid gap-4 xl:grid-cols-2">
      {Array.from({ length: 6 }).map((_, index) => (
        <div key={index} className="h-72 animate-pulse rounded-[26px] border border-white/70 bg-white/55 p-5 shadow-[0_20px_65px_rgba(90,47,43,0.08)]">
...
function EmptyState({ hasOrders }: { hasOrders: boolean }) {
  return (
    <section className="rounded-[30px] border border-white/75 bg-white/60 p-10 text-center shadow-[0_22px_70px_rgba(90,47,43,0.1)] backdrop-blur-2xl">
      ...
      <h2 className="mt-6 font-serif text-2xl font-bold text-[#3f231f]">{hasOrders ? "No orders match your filters" : "No bakery orders yet"}</h2>
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#76524c]">
        {hasOrders ? "Try clearing the search, status, or date filter to see more orders." : "New cake orders will appear here automatically with sound and toast notifications."}
      </p>
    </section>
  );
}
      ...
      {toasts.map((toast) => (
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <button type="button" onClick={() => onDismiss(toast.id)} className="rounded-full p-1 transition hover:bg-black/5" aria-label="Dismiss notification">
              <X size={16} />
            </button>
          </div>
        </div>
...
function ConfirmModal({ dialog, onCancel }: { dialog: ConfirmDialog | null; onCancel: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[90] grid place-items-center bg-[#2d1714]/35 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[28px] border border-white/70 bg-white p-6 text-[#3f231f] shadow-[0_30px_100px_rgba(45,23,20,0.24)]">
        ...
        <p className="mt-3 text-sm leading-6 text-[#76524c]">{dialog.message}</p>
        <div className="mt-6 flex justify-end gap-3">
          <button type="button" onClick={onCancel} className="rounded-2xl border border-[#5a2f2b]/10 px-4 py-2 text-sm font-bold text-[#5a2f2b] transition hover:bg-[#5a2f2b]/5">
            Cancel
          </button>
          <button
            type="button"
            ...
            className={`rounded-2xl px-4 py-2 text-sm font-bold text-white transition hover:-translate-y-0.5 ${
              dialog.tone === "danger" ? "bg-red-600 shadow-lg shadow-red-500/20" : "bg-[#5a2f2b] shadow-lg shadow-[#5a2f2b]/20"
            }`}
          >
            {dialog.confirmLabel}
          </button>
