...
const galleryImages = [
  ...
  {
    id: 3,
    src: "/images/red_velvet_cake.png",
    title: "Velvet Love",
    size: "medium",
    category: "Red Velvet",
  },
      ...
      <div className="container">
        <div className="text-center mb-16">
          ...
          <p className="text-chocolate/60 max-w-xl mx-auto lead">
            A visual feast of our most beloved creations. Each slice tells a
            story of meticulous craftsmanship and sweet inspiration.
          </p>
        </div>
        <div className="mb-8 flex items-center justify-center gap-3">
          <button
            onClick={() => setFilter("All")}
            className={`px-4 py-2 rounded-full ${filter === "All" ? "btn-primary" : "btn-secondary"}`}
          >
            All
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full ${filter === cat ? "btn-primary" : "btn-secondary"}`}
            >
              {cat}
            </button>
          ))}
...
function LightboxWrapper({ images, index, onClose, onChangeIndex }) {
  // Manage keyboard navigation with proper effect and cleanup
  useEffect(() => {
    const handleKey = (e) => {
      if (e.key === "Escape") onClose();
      if (e.key === "ArrowRight")
        onChangeIndex(Math.min(images.length - 1, index + 1));
  ...
  return (
    ...
    >
      ...
      <button
        className="absolute left-6 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 text-cream"
        onClick={(e) => {
          e.stopPropagation();
      ...
      <button
        className="absolute right-6 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/10 text-cream"
        onClick={(e) => {
          e.stopPropagation();
          onChangeIndex(Math.min(images.length - 1, index + 1));
        }}
