...
function validate(body: CreateOrderBody) {
  const errors: string[] = [];
  ...
  if (!body.quantity || typeof body.quantity !== 'number') errors.push('quantity must be a number');
  if (!body.deliveryAddress) errors.push('deliveryAddress is required');
  if (body.totalPrice === undefined || typeof body.totalPrice !== 'number') errors.push('totalPrice must be a number');
  return errors;
}
...
export async function GET() {
  try {
    ...
    const mapped = (docs as LeanOrder[]).map((d) => ({ ...d, id: d._id.toString() }));
